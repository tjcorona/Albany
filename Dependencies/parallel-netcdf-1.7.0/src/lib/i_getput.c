              ...
              int               *reqid)
{
    ...
    if (reqid != NULL) *reqid = NC_REQ_NULL;
    status = ncmpii_sanity_check(ncid, varid, NULL, NULL, bufcount, API_VAR,
                                 0, 1, READ_REQ, NONBLOCKING_IO, &ncp, &varp);
    if (status != NC_NOERR) return status;
                 ...
                 int             *reqid)
{
    ...
    /* iput_var is a special case of iput_varm */
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_SIGNED_CHAR, reqid,
                                 WRITE_REQ, 0, 0);
    NCI_Free(start);
    return status;
}
                 ...
                 int             *reqid)
{
    ...
    /* iput_var is a special case of iput_varm */
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED_CHAR, reqid,
                                 WRITE_REQ, 0, 0);
    NCI_Free(start);
    return status;
}
                 ...
                 int             *reqid)
{
    ...
    /* iput_var is a special case of iput_varm */
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED_SHORT, reqid,
                                 WRITE_REQ, 0, 0);
    NCI_Free(start);
    return status;
}
                 ...
                 int             *reqid)
{
    ...
    /* iput_var is a special case of iput_varm */
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_LONG_LONG_INT, reqid,
                                 WRITE_REQ, 0, 0);
    NCI_Free(start);
    return status;
}
                 ...
                 int             *reqid)
{
    ...
    /* iput_var is a special case of iput_varm */
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED_LONG_LONG, reqid,
                                 WRITE_REQ, 0, 0);
    NCI_Free(start);
    return status;
}
                 ...
                 int             *reqid)
{
    ...
    if (reqid != NULL) *reqid = NC_REQ_NULL;
    status = ncmpii_sanity_check(ncid, varid, NULL, NULL, 0, API_VAR,
                                 0, 0, READ_REQ, NONBLOCKING_IO, &ncp, &varp);
    if (status != NC_NOERR) return status;
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_CHAR, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_SIGNED_CHAR, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED_CHAR, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_SHORT, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED_SHORT, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_INT, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_LONG, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_FLOAT, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_DOUBLE, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_LONG_LONG_INT, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED_LONG_LONG, reqid, WRITE_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_CHAR, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_SIGNED_CHAR, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED_CHAR, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_SHORT, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED_SHORT, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_INT, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_LONG, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_FLOAT, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_DOUBLE, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_LONG_LONG_INT, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    status = ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                                 (void*)buf, -1, MPI_UNSIGNED_LONG_LONG, reqid, READ_REQ, 0,
                                 0);
    NCI_Free(count);
    return status;
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_CHAR, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_SIGNED_CHAR, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_UNSIGNED_CHAR, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_SHORT, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_UNSIGNED_SHORT, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_INT, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_UNSIGNED, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_LONG, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_FLOAT, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_DOUBLE, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_LONG_LONG_INT, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_UNSIGNED_LONG_LONG, reqid, WRITE_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_CHAR, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_SIGNED_CHAR, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_UNSIGNED_CHAR, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_SHORT, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_UNSIGNED_SHORT, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_INT, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_UNSIGNED, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_LONG, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_FLOAT, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_DOUBLE, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_LONG_LONG_INT, reqid, READ_REQ, 0, 0);
}
                  ...
                  int              *reqid)
{
    ...
    return ncmpii_igetput_varm(ncp, varp, start, count, NULL, NULL,
                               (void*)buf, -1, MPI_UNSIGNED_LONG_LONG, reqid, READ_REQ, 0, 0);
}
