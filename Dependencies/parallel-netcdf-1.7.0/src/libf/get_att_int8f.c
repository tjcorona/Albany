/* -*- Mode: C; c-basic-offset:4 ; -*- */
/*  
 *  (C) 2003 by Argonne National Laboratory and Northwestern University.
 *      See COPYRIGHT in top-level directory.
 *
 * This file is automatically generated by ./buildiface -infile=../lib/pnetcdf.h -deffile=./defs
 * DO NOT EDIT
 */
#include "mpinetcdf_impl.h"


#ifdef F77_NAME_UPPER
#define nfmpi_get_att_int8_ NFMPI_GET_ATT_INT8
#elif defined(F77_NAME_LOWER_2USCORE)
#define nfmpi_get_att_int8_ nfmpi_get_att_int8__
#elif !defined(F77_NAME_LOWER_USCORE)
#define nfmpi_get_att_int8_ nfmpi_get_att_int8
/* Else leave name alone */
#endif


/* Prototypes for the Fortran interfaces */
#include "mpifnetcdf.h"
FORTRAN_API int FORT_CALL nfmpi_get_att_int8_ ( int *v1, int *v2, char *v3 FORT_MIXED_LEN(d3), long long * v4 FORT_END_LEN(d3) ){
    int ierr;
    int l2 = *v2 - 1;
    char *p3;

    {char *p = v3 + d3 - 1;
     int  li;
        while (*p == ' ' && p > v3) p--;
        p++;
        p3 = (char *)malloc( (size_t)(p-v3) + 1 );
        for (li=0; li<(p-v3); li++) { p3[li] = v3[li]; }
        p3[li] = 0; 
    }
    ierr = ncmpi_get_att_longlong( *v1, l2, p3, v4 );
    free( p3 );
    return ierr;
}
