...
#ifndef ROTATIONMECHANICSPROBLEM_HPP
#define ROTATIONMECHANICSPROBLEM_HPP
...
static int rot_dir_count = 0; //counter for registration of dirichlet_field
...
///
class RotationMechanicsProblem: public Albany::AbstractProblem
{
public:
  ...
  /// Default constructor
  ///
  RotationMechanicsProblem(const Teuchos::RCP<Teuchos::ParameterList>& params,
...
private:
  ...
  ///
  RotationMechanicsProblem(const RotationMechanicsProblem&);

  ///
  /// Private to prohibit copying
  ///
  RotationMechanicsProblem& operator=(const RotationMechanicsProblem&);
...
#include "RotatingReferenceFrame.hpp"
#include "Density.hpp"
    ...
    const Teuchos::RCP<Teuchos::ParameterList>& responseList)
{
  ...
  *out << "In RotationMechanicsProblem::constructEvaluators" << std::endl;
...
//  }
  ...
  *out << "Field Dimensions: Workset=" << workset_size
  ...
  { // Rotating Reference Frame
    ...
    //   rcp(new Teuchos::ParameterList("Rotating Reference Frame"));
    ...
    p->set<Teuchos::RCP<PHX::DataLayout>>( "QP Scalar Data Layout",
                                           dl_->qp_scalar);
    p->set<Teuchos::RCP<PHX::DataLayout>>( "QP Vector Data Layout",
                                           dl_->qp_vector);
    p->set<Teuchos::RCP<ParamLib>>("Parameter Library", paramLib);
    Teuchos::ParameterList& paramList = params->sublist("Rotating Reference Frame");
    p->set<Teuchos::ParameterList*>("Parameter List", &paramList);

    // if(material_db_ != Teuchos::null)
    //   p->set< Teuchos::RCP<QCAD::MaterialDatabase> >("MaterialDB", material_db_);

    // Output
    p->set<std::string>("Force Name", "Force");
    ev = Teuchos::rcp(
      new AFRL::RotatingReferenceFrame<EvalT, PHAL::AlbanyTraits>(*p, dl_));
    fm0.template registerEvaluator<EvalT>(ev);
  }
  ...
  { // Density
    ...
    p->set<Teuchos::RCP<PHX::DataLayout>>("Cell Scalar Data Layout", dl_->cell_scalar2);

    p->set<Teuchos::RCP<ParamLib> >("Parameter Library", paramLib);
    Teuchos::ParameterList& paramList = params->sublist("Density");
    p->set<Teuchos::ParameterList*>("Parameter List", &paramList);

    ev = Teuchos::rcp(new LCM::Density<EvalT,PHAL::AlbanyTraits>(*p));
    fm0.template registerEvaluator<EvalT>(ev);
  }
  ...
  if (rot_dir_count == 0){ //constructEvaluators gets called multiple times for different specializations.
